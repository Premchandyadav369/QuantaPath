...
export class DistanceService {
  ...
  async calculateMatrix(request: DistanceMatrixRequest): Promise<DistanceMatrix> {
    ...
    // The `source` parameter can be used to force a specific provider.
    // Otherwise, we default to the Google -> OpenRouteService -> Haversine fallback chain.
    switch (request.source) {
      ...
      case "google":
      default:
        matrix = await this.calculateGoogleMatrix(request); // This method contains the fallback logic
        break;
    }
  ...
  async getDetailedRoute(stops: DeliveryStop[], tour: number[]): Promise<DetailedRoute> {
    // Default to Google for detailed routes, with fallback to OpenRouteService
    return this.getGoogleDetailedRoute(stops, tour);
  }

  private async getGoogleDetailedRoute(stops: DeliveryStop[], tour: number[]): Promise<DetailedRoute> {
    const apiKey = process.env.GOOGLE_MAPS_API_KEY;

    if (!apiKey) {
      console.warn("Google Maps API key not found, falling back to OpenRouteService for detailed route.");
      return this.getOpenRouteDetailedRoute(stops, tour);
    }

    try {
      const orderedStops = tour.map(i => stops[i]);
      const origin = orderedStops[0];
      const destination = orderedStops[orderedStops.length - 1];
      const waypoints = orderedStops.slice(1, -1).map(stop => `${stop.lat},${stop.lng}`).join('|');
      ...
      return this.parseGoogleDirectionsResponse(data, orderedStops);
    } catch (error) {
      console.warn("Google Directions API failed, falling back to OpenRouteService:", error);
      return this.getOpenRouteDetailedRoute(stops, tour);
    }
  }

  private parseGoogleDirectionsResponse(data: any, stops: DeliveryStop[]): DetailedRoute {
    const route = data.routes[0];
    if (!route) {
      throw new Error("No route found in Google Directions response");
    }

    const overviewGeometry = decodePolyline(route.overview_polyline.points);

    let totalDistance = 0;
    let totalDuration = 0;

    const segments: RouteSegment[] = route.legs.map((leg: any, i: number) => {
      totalDistance += leg.distance.value;
      totalDuration += leg.duration.value;

      const legGeometry: [number, number][] = leg.steps.flatMap((step: any) =>
        decodePolyline(step.polyline.points)
      );

      const instructions: TurnInstruction[] = leg.steps.map((step: any) => ({
      ...
      return {
        ...
        instructions: instructions.slice(0, 5), // Limit instructions per segment
      };
    });
  ...
  private async getOpenRouteDetailedRoute(stops: DeliveryStop[], tour: number[]): Promise<DetailedRoute> {
    const apiKey = process.env.OPENROUTE_API_KEY;
    ...
    try {
      ...
      return this.parseOpenRouteDirectionsResponse(data, stops, tour);
    } catch (error) {
  ...
  private parseOpenRouteDirectionsResponse(data: any, stops: DeliveryStop[], tour: number[]): DetailedRoute {
